...
class SuperBizAgentApp {
    ...
    initializeElements() {
        ...
        this.experimentAnalysisSidebarBtn = document.getElementById('experimentAnalysisSidebarBtn');
    ...
    bindEvents() {
        ...
        // 实验分析按钮
        if (this.experimentAnalysisSidebarBtn) {
            this.experimentAnalysisSidebarBtn.addEventListener('click', () => this.triggerExperimentAnalysis());
        }
    ...
    updateUI() {
        ...
        if (this.messageInput) {
            ...
            this.messageInput.placeholder = '询问土木实验相关问题';
        }
    }
    ...
    // 发送实验异常分析请求（SSE 流式模式）
    async sendExperimentAnalysisRequest(loadingMessageElement) {
        try {
            ...
            try {
                while (true) {
                    ...
                    if (done) {
                        // 流结束，更新最终内容
                        if (fullResponse) {
                            console.log('实验分析 流结束，更新最终内容，长度:', fullResponse.length);
                            this.updateExperimentAnalysisMessage(loadingMessageElement, fullResponse, []);
                        }
                        break;
                    }
                    ...
                    for (const line of lines) {
                        ...
                        if (line.startsWith('id:')) {
                            ...
                            currentEvent = line.substring(6).trim();
                            console.log('[实验分析 SSE] 事件类型:', currentEvent);
                            continue;
                        } else if (line.startsWith('data:')) {
                            const rawData = line.substring(5).trim();
                            console.log('[实验分析 SSE] 数据:', rawData, ', currentEvent:', currentEvent);
                            ...
                            const processJsonMessages = (data) => {
                                ...
                                if (matches && matches.length > 0) {
                                    console.log('[实验分析 SSE] 匹配到', matches.length, '个JSON对象');
                                    for (const jsonStr of matches) {
                                        try {
                                            ...
                                            if (sseMessage.type === 'content') {
                                                ...
                                                console.log('实验分析 流完成，最终内容长度:', fullResponse.length);
                                                this.updateExperimentAnalysisMessage(loadingMessageElement, fullResponse, []);
                                                return true;
                                            } else if (sseMessage.type === 'error') {
                                                throw new Error(sseMessage.data || '实验异常分析分析失败');
                                            }
                                        } catch (e) {
                                            if (e.message.includes('实验异常分析')) throw e;
                                            console.log('[实验分析 SSE] 单个JSON解析失败:', jsonStr);
                                        }
                                    }
                                    if (loadingMessageElement) {
                                        this.updateExperimentAnalysisStreamContent(loadingMessageElement, fullResponse);
                                    }
                                    return false;
                                }
                                return null;
    ...
    // 更新实验异常分析流式内容（实时显示）
    updateExperimentAnalysisStreamContent(messageElement, content) {
        if (!messageElement) return;
        
        // 添加 experiment-analysis-message 类
        messageElement.classList.add('experiment-analysis-message');
    ...
    // 更新实验异常分析消息（带折叠详情）
    updateExperimentAnalysisMessage(messageElement, response, details) {
        console.log('updateExperimentAnalysisMessage 被调用');
        ...
        if (!messageElement) {
            ...
            return this.addExperimentAnalysisMessage(response, details);
        }

        // 添加experiment-analysis-message类
        messageElement.classList.add('experiment-analysis-message');
    ...
    // 添加实验异常分析消息（带折叠详情）- 保留用于兼容性
    addExperimentAnalysisMessage(response, details) {
        const messageDiv = document.createElement('div');
        messageDiv.className = 'message assistant experiment-analysis-message';
    ...
    // 触发实验异常分析（点击实验异常分析按钮时直接调用）
    async triggerExperimentAnalysis() {
        if (this.isStreaming) {
        ...
        this.currentExperimentAnalysisMessage = loadingMessage; // 保存消息引用用于后续更新
        ...
        try {
            await this.sendExperimentAnalysisRequest(loadingMessage);
        } catch (error) {
            console.error('实验异常分析分析失败:', error);
            // 更新消息为错误信息
            if (loadingMessage) {
                const messageContent = loadingMessage.querySelector('.message-content');
                if (messageContent) {
                    messageContent.textContent = '抱歉，实验异常分析分析时出现错误：' + error.message;
                }
            }
        } finally {
            this.isStreaming = false;
            this.currentExperimentAnalysisMessage = null;
            this.updateUI();
        }
    }
    ...
    showLoadingOverlay(show) {
        if (this.loadingOverlay) {
            if (show) {
                ...
                if (loadingText) loadingText.textContent = '实验异常分析分析中，请稍候...';
