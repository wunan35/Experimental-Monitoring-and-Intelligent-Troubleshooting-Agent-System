...
public class VectorIndexService {
    ...
    public void indexSingleFile(String filePath) throws Exception {
        ...
        if ("semantic".equalsIgnoreCase(strategy)) {
            logger.info("使用语义分片策略");
            chunks = semanticChunkService.chunkDocumentSemantically(content, path.toString());
        } else if ("hybrid".equalsIgnoreCase(strategy)) {
            logger.info("使用混合分片策略（暂未实现，使用传统分片）");
            chunks = chunkService.chunkDocument(content, path.toString());
        } else {
            logger.info("使用传统分片策略");
            chunks = chunkService.chunkDocument(content, path.toString());
        }
