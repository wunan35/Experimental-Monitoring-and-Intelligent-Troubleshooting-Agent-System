...
start:
	...
	@if curl -s -f $(HEALTH_CHECK_API) > /dev/null 2>&1; then \
		...
		if [ ! -f target/super-biz-agent-1.0-SNAPSHOT.jar ]; then \
			echo "$(YELLOW)📦 正在打包应用...$(NC)"; \
			mvn clean package -DskipTests > /tmp/build.log 2>&1; \
			if [ $$? -ne 0 ]; then \
				echo "$(RED)❌ 打包失败，请检查日志: /tmp/build.log$(NC)"; \
				exit 1; \
			fi; \
			echo "$(GREEN)✅ 应用打包完成$(NC)"; \
		fi; \
		nohup java -jar target/super-biz-agent-1.0-SNAPSHOT.jar > server.log 2>&1 & \
		echo $$! > server.pid; \
...
stop:
	...
	@if [ -f server.pid ]; then \
		...
		pkill -f "spring-boot:run" && echo "$(GREEN)✅ 已停止所有 spring-boot 进程$(NC)" || pkill -f "super-biz-agent" && echo "$(GREEN)✅ 已停止所有 java 进程$(NC)" || echo "$(YELLOW)⚠️  没有运行中的 spring-boot 或 java 进程$(NC)"; \
	fi
